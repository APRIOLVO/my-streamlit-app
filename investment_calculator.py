...
NumberConsumers = st.number_input("Number of Users ($K)", min_value=0.0, step=1.0) * 1000
...
def ValueTotal(InvestmentAmount, ValuePerDollar):
...
if st.button("Calculate"):
    ...
    total_value = ValueTotal(InvestmentAmount, value_per_dollar)
    ...
    st.metric(label="Total Profit of Investment", value=f"${total_value:,.0f}")
